 ...
 * 읽힌다.
 *
 * ⚠ 처음에 3초로 뒀는데 **짧았다.** 이 그림은 훑을 것이 많다 — 배·고양이 둘·해·구름,
 *   그리고 배 아래 주인공 아이콘 둘(§7.5 의 회수)까지 눈이 닿아야 한다. 크로스페이드
 *   0.65초를 빼면 실제로 보는 시간이 2.3초뿐이라 "지나갔다"가 된다.
 *   7초는 화면이 멈춘 줄 알기 직전까지다 — 더 늘리려면 그때는 정적이 아니라 뭔가
 *   움직이는 것(scenefx)을 붙일 자리다.
 */
const ENDING_HOLD_MS = 7000;
