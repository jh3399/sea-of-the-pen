...
export const HYDRO_TUNING = {
  ...
  dragBase: 1.0,
  /**
   * ★ 저속 항력 기준 속도 (m/s) — **"미끄러진다"를 없애는 노브.**
   *
   * 항력을 순수한 v² 로만 두면 `dv/dt ∝ −v²` 라 감쇠가 **1/t 로 꼬리를 끈다.** 실측: 종단
   * 4.66 m/s 에서 노를 놓은 배가 30초 동안 69 m 를 더 나아가고도 아직 20% 아래로 안 떨어졌다.
   * 빠를 때는 잘 서는데 느려질수록 물이 손을 놓는 셈이라, 조작으로는 "배가 얼음 위를 탄다"로
   * 읽힌다. dragBase 를 올려 봐야 계수만 커질 뿐 꼬리의 **모양**은 그대로다 (2.0 에서도 44 m).
   *
   * 그래서 항력을 `drag·(|v| + v₀)·v` 로 읽는다 — v₀ 아래에서는 선형 항이 이겨 **지수 감쇠**가
   * 되고(시정수 m/(drag·v₀)), v₀ 위에서는 여전히 v² 다. 점성 항력이 저레이놀즈에서 선형이
   * 되는 것과 같은 이야기이고, 무엇보다 **분기 없이 한 항**으로 두 영역이 나온다.
   *
   * 종단 속도는 이 값에 거의 안 움직인다 (고속에서 |v| ≫ v₀). 움직이는 것은 "놓았을 때
   * 언제 서는가"뿐이다.
   */
  dragLowSpeed: 1.4,
  /**
 ...
 * (흘수 3배 → 노 종단 4.24 → 3.33 m/s). 나무는 얕은 흘수가 강점이고, 각도로 도망칠 수 없어
...
export const MATERIALS = {
  ...
  /**
   * 힘줄. **살과 같은 이유로 플레이어가 못 고른다** (`PALETTE_MATERIALS` 하드코딩) — 불가사리의
   * 팔이 이걸 쓴다. 규칙표에 줄이 없는 것도 살과 같다(자기 빔에 자기가 안 녹는다, §7 원칙 1).
   *
   * ★ 살과 다른 것은 여전히 `maxCarveRadius` 하나뿐이지만, **용도가 완전히 바뀌었다.**
   *   한동안(2026-08-10 세 번째~네 번째 라운드) 팔은 실제로 깎이는 지형이라 이 값이 "몇
   *   발이면 한 조각이 끊겨 나가는가"를 정하는 진짜 형상 캡이었다 — 0.5(비현실적, 정밀
   *   사격도 십수 발) → 1.0(팔 끝 폭 1.6 m 보다 지름이 커서 한 발에 즉발) → 0.75(가장
   *   좁은 끝보다 지름이 작아 최소 두 발) 순으로 조절했었다.
   *
   *   ★ 그 뒤 "보스 형태 전체가 안 부서지게"(다섯 번째 라운드)로 팔도 핵과 같은 무형태
   *   진행 지표가 됐다 — 폴리곤은 스폰된 그대로 영원히 고정이고, `maxCarveRadius` 는 이제
   *   **형상을 자르는 데 전혀 안 쓰인다.** 대신 `game/boss.js#applyDamage()` 가 쓰는 근사
   *   피해 면적(`π·radius²`) 산식에서 "발당 최대 피해"만 정하는 순수 밸런스 노브로
   *   남았다 — 핵(`flesh`, 캡 0.6)보다 팔(캡 0.75)이 발당 피해가 더 크다는 뜻이다.
   *   0.75 라는 값 자체는 이전 라운드에서 고른 것을 그대로 물려받았을 뿐, 새로 정한
   *   근거는 없다 — 밸런스를 다시 만지고 싶으면 이 숫자부터 건드리면 된다.
   */
  sinew: {
    key: 'sinew', name: '힘줄', areaDensity: 220, color: '#9c2f6e',
    impactThreshold: 4000, toughness: 20000, maxCarveRadius: 0.75,
    deflection: 0,
  },
  /**
