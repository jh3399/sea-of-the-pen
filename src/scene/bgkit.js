...
// ---------------- 만년필 ----------------
//
// ★ 이 게임에서 유일하게 **두 군데에 같은 모양으로 나와야 하는 물건**이다 —
//   S-04 의 `the_pen` 씬(모루가 꺼내는 펜)과 설계 화면의 커서. 둘이 다르게 생기면
//   "모루가 준 펜으로 그린다"는 연결이 끊긴다. 그래서 모양을 여기 한 번만 정의하고,
//   커서 쪽(draw.css)의 SVG 는 이 비율을 그대로 베낀다.
//
// 좌표계: (x, y) = 몸통 왼쪽 위. 전체 길이 92s, 두께 11s, **펜촉은 오른쪽**이다.
// 도트 규칙상 회전은 쓰지 않는다 (fillRect 를 돌리면 가장자리가 뭉갠다).

export const PEN_PALETTE = {
  body: '#16202e', bodyLit: '#2c4360', bodyDark: '#0b111a',
  gold: '#e8c46a', goldDark: '#a8813a', goldLit: '#f7e6ae',
  sea: '#5fd6e8',   // 바다에서 주워 온 물건이라는 표시. 몸통에 실낱만큼만 남는다.
};

/**
 * 만년필 한 자루. s = 픽셀 배율 (1이면 86×14, 펜촉 끝이 (86s, 7.5s)).
 *
 * ⚠ 커서로도 쓰이므로 **펜촉 끝의 y 는 반드시 몸통 중심**이어야 한다 (7행).
 *   커서 hotspot 을 촉 끝에 두는데, 그 점이 몸통 중심이 아니면 그리는 자리와
 *   커서 그림이 어긋난 것처럼 보인다.
 */
export function fountainPen(ctx, x, y, s = 1, pal = {}) {
  const P = { ...PEN_PALETTE, ...pal };
  const F = (a, b, c, d, col) => fill(ctx, x + a * s, y + b * s, c * s, d * s, col);

  // 몸통 — 3톤. 광원은 좌상단이라 윗면이 밝고 아랫면이 그늘이다.
  F(3, 3, 49, 9, P.body);
  F(3, 3, 49, 2, P.bodyLit);
  F(3, 10, 49, 2, P.bodyDark);
  F(20, 6, 20, 1, P.sea);          // 청록 실선 한 줄 — 바다에서 왔다는 표시. 이보다 굵으면 형광등이 된다.

  F(0, 4, 4, 7, P.goldDark);       // 꼬리 마개
  F(0, 4, 4, 2, P.gold);

  F(14, 1, 19, 2, P.gold);         // 클립
  F(14, 1, 2, 4, P.goldDark);

  F(52, 2, 5, 11, P.gold);         // 목띠 — 몸통보다 한 칸씩 넓어 마디가 보인다
  F(52, 2, 5, 2, P.goldLit);
  F(52, 11, 5, 2, P.goldDark);

  // 그립 — 촉으로 갈수록 가늘어진다. 계단 둘로만 끊어야 도트에서 매끄럽다.
  F(57, 4, 5, 7, P.body);
  F(57, 4, 5, 2, P.bodyLit);
  F(62, 5, 4, 5, P.body);
  F(62, 5, 4, 1, P.bodyLit);

  // 펜촉 — 넓게 시작해 세 단으로 좁힌다. 바늘처럼 가늘면 펜으로 안 읽힌다.
  F(66, 4, 8, 7, P.gold);
  F(74, 5, 6, 5, P.gold);
  F(80, 6, 4, 3, P.gold);
  F(84, 7, 2, 1, P.goldLit);       // 끝 — y=7 이 몸통 중심이다 (커서 hotspot)
  F(66, 4, 8, 1, P.goldLit);       // 윗면 하이라이트
  F(74, 5, 6, 1, P.goldLit);
  F(66, 10, 8, 1, P.goldDark);     // 아랫면 그늘
  F(69, 6, 2, 2, P.bodyDark);      // 숨구멍
  F(71, 7, 13, 1, P.goldDark);     // 슬릿 — 숨구멍에서 촉 끝까지
}
